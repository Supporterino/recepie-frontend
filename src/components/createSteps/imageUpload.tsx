...
import { Button, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, Input, Typography } from '@mui/material';
...
const ImageUpload: React.FunctionComponent<ImageUploadProps> = ({ open, close, target, recipeID }: ImageUploadProps) => {
    ...
    const uploadMutation = useMutation((data: FormData) => sendRequest(imageUploadUrl, 'POST', data, false), {
        onSuccess: () => {
            return Promise.all([
    ...
    const upload = () => {
        if (hasFile()) {
            ...
            if (target === PhotoTypes.RECIPE || target === PhotoTypes.ADDITION_RECIPE) data.append('recipeID', recipeID!);
