...
const AddIngredient: React.FunctionComponent<AddIngredientProps> = ({ open, close, updateData }: AddIngredientProps) => {
    ...
    ['mass', 'volume', 'each'].forEach((option: string) => {
        allConverter.possibilities(option as AllMeasures).forEach(unit => options.push({ category: option, unit }));
    });
