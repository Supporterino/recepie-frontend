...
import sendRequest, { addCookListUrl, addFavoriteUrl, removeCookListUrl, removeFavoriteUrl } from '../../services/requestService';
...
const CardWide: React.FunctionComponent<CardWideProps> = ({ recipe }: CardWideProps) => {
    ...
    return (
        <Card sx={{ display: 'flex', my: 1, boxShadow: 10, borderRadius: '4px' }} raised>
            ...
            <RecipeImage url={recipe.picture} width="120px" height="150px" onClick={() => navigate(`/recipe/${recipe.id}`)} />
        </Card>
    );
};
